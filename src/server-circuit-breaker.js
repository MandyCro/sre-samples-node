...
async function externalService() {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const shouldFail = Math.random() > 0.9 // Simula o percentual de falhas
      if (shouldFail) {
...
breaker.on('reject', () =>
  console.log('Requisição rejeitada pelo Circuit Breaker'),
)
breaker.on('failure', () =>
  console.log('❌ Falha registrada pelo Circuit Breaker'),
)
breaker.on('success', () =>
  console.log('✅ Sucesso registrado pelo Circuit Breaker'),
)
...
async function testCircuitBreaker() {
  ...
  const interval = setInterval(async () => {
    await fetch('http://localhost:8080/api/circuitbreaker')
